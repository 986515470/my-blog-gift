...
//             $("。timeline").html("");
            $.each(data,function(i,k){
             ...
             var html="";
                html +='<div style="border-bottom:1px dashed #000000;margin-bottom: 35px;margin-top: -25px;opacity: 0.2 ;"></div>';
                html +='<li class="event">';
                if(i==0)
                  html +='	<input type="radio" name="tl-group" checked/>';
                else
                  html +='	<input type="radio" name="tl-group" />';
                html +='	<label></label>';
                if(k.name=="Cy")
                html +='	<div class="thumb user-4"><span>'+k.time+'</span></div>';
                else
                 html +='	<div class="thumb user-6"><span>'+k.time+'</span></div>';
                html +='	<div class="content-perspective">';
                html +='		<div class="content">';
                html +='			<div class="content-inner">';
                html +='				<h3>'+k.things+'️&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+k.describe+'</h3>';
                html +='			</div>';
                html +='		</div>';
                html +='	</div>';
                html +='</li>';
...
//             $("#look").remove();
           var html = '';
           for(var i in data){
             html += "<li data=\""+data[i].id+"\">x<span>"+data[i].id+"."+data[i].things+"</span></li>";//拼接标签
           }
            html += "<div style="+"border-bottom:1px dashed #000000;margin-bottom:10px;opacity: 0.2 ;"+">---------------以下已打卡---------------</div>";
...
$(function(){
  ...
  $(".label-item").on("click","li",function(){
    ...
    if($(this).hasClass("selected")){
      return false;
    }else if($(".label-selected").children("li").length >= 1){
      layer.msg("技术原因，目前先最多可以选择1个哦");
      return false;
    }
...
//
//        for(var i in data){
//          html += "<li data=\""+data[i].id+"\">x<span>"+data[i].things+"</span></li>";//拼接标签
//        }
//        $(".label-item").append(html);//追加至文档
//      },
//      error:function(){
...
//  })
  ...
  $("#cell").on("click",function(){
  	if($("input[name='label']").val() == ""){
		...
		layer.msg("打卡成功："+$("input[name='label']").val());
		   			  var aaa=$("input[name='label']").val();
//		   			  $("input[type=hidden]").val(aaa);
					  // alert(aaa);
//		              document.getElementById('label').value=aaa;
               $('#label').html(aaa);
               $('#label').val(aaa);
